    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          ...
          <p className="text-4xl font-bold text-green-400 mt-2">{summary?.posture_score}/100</p>
        </div>
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <h2 className="text-lg text-gray-400">Critical Risks</h2>
          <p className="text-4xl font-bold text-red-500 mt-2">{summary?.critical_risks_count}</p>
        </div>
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <h2 className="text-lg text-gray-400">High Risks</h2>
          <p className="text-4xl font-bold text-orange-400 mt-2">{summary?.high_risks_count}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
          {summary?.top_findings.map((f, i) => (
            <div key={i} className="p-4 border-b border-gray-700 last:border-0 flex justify-between items-center hover:bg-gray-750">
              <div className="flex items-center gap-4">
                <AlertTriangle className={f.severity === 'Critical' ? 'text-red-500' : 'text-orange-500'} />
                <div>
                  <h3 className="font-semibold text-white">{f.title}</h3>
                  <p className="text-sm text-gray-400">{f.source}</p>
                </div>
              </div>
            </div>
          ))}
